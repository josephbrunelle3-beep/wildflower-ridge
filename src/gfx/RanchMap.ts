...
/**
 * Paints the ranch layout and returns it in Tiled JSON format, so the game needs no map
 * asset on disk. `scripts/build-map.ts` writes the same object out as ranch.json if you
 * ever want to open or edit it in Tiled; loading a hand-authored map instead only needs
 * the layer names (`ground`, `decor`, `objects`) to match.
 */
...
export function buildRanchMap(): RanchMap {
  const W = MAP_W;
  const H = MAP_H;
  ...
  const addPoint = (name: string, type: string, tx: number, ty: number, properties?: MapObject['properties']) =>
    objects.push({ id: nextId++, name, type, x: px(tx), y: px(ty), width: TILE_SIZE, height: TILE_SIZE, properties });
  ...
  // Deterministic pseudo-random so the ranch looks the same every run.
  let seed = 1337;
  ...
  // --- North pasture (fenced, gate at the bottom) ---------------------------
  ...
  fillG(30, 27, 38, 27, TILE.DIRT);
  setD(39, 27, -1); setG(39, 27, TILE.DIRT); // gap in the forest = road out of the valley
  fillG(29, 21, 31, 21, TILE.DIRT);
  ...
  const tileLayer = (id: number, name: string, g: Grid): TileLayer => ({
